...
def add_predictions(input_data):
    model = pickle.load(open('D:\mpl\Streamlit_Project\model\model.pkl', 'rb'))
    scalar = pickle.load(open('D:\mpl\Streamlit_Project\model\scalar.pkl','rb'))
    ...
    if prediction[0] == 0 :
        st.markdown('<h3 style="color:green;">Begnine</h3>', unsafe_allow_html=True)
    else:
        st.markdown('<h3 style="color:red;">Malicious</h3>', unsafe_allow_html=True)
...
def main():
    st.set_page_config(
        page_title="BreastCancerPredictor",
        layout="wide",
    ...
    with st.container():
        ...
        st.write("This application predicts the presence of breast cancer provided with all the various types of inputs actually results from tissue sample test.")
    ...
    with col2:
        with st.container(key=None, border=True):
